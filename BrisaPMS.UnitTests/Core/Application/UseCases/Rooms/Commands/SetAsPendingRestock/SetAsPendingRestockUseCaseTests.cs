...
public class SetAsPendingRestockUseCaseTests
{
  private readonly IRoomsRepository _roomsRepositoryMock;
  private readonly IUnitOfWork _unitOfWorkMock;
  private readonly SetAsPendingRestockUseCase _useCase;

  public SetAsPendingRestockUseCaseTests()
  {
    _roomsRepositoryMock = Substitute.For<IRoomsRepository>();

    _unitOfWorkMock = Substitute.For<IUnitOfWork>();

    _useCase = new SetAsPendingRestockUseCase(_roomsRepositoryMock, _unitOfWorkMock);
  }

  [Fact]
  public async Task Handle_SetsRoomAsPendingRestockAndReturnsTrue()
  {
    // Arrange
  ...
  private static SetAsPendingRestockCommand CreateCommand(Guid roomId)
  {
