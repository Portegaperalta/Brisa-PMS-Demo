...
public class UpdateCompanyContactInfoUseCaseTests
{
    private readonly ICompaniesRepository _repositoryMock;
    private readonly IUnitOfWork _unitOfWorkMock;
    private readonly UpdateCompanyContactInfoUseCase _useCase;

    public UpdateCompanyContactInfoUseCaseTests()
    {
        _repositoryMock = Substitute.For<ICompaniesRepository>();
        _unitOfWorkMock = Substitute.For<IUnitOfWork>();
        _useCase = new UpdateCompanyContactInfoUseCase(_repositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task Handle_UpdatesCompanyContactInfo()
    {
        var companyId = Guid.NewGuid();
        var company = CreateCompany(companyId);
        var newBusinessEmail = "newEmail@brisa.com";
        var newBusinessPhoneNumber = "+19817779999";
        var command = CreateCommand(companyId, newBusinessEmail, newBusinessPhoneNumber);
        ...
        company.BusinessEmail.Value.Should().Be(newBusinessEmail);
        company.BusinessPhone.Value.Should().Be(newBusinessPhoneNumber);
    ...
    {
        var companyId = Guid.NewGuid();
        var command = CreateCommand(companyId, CreateBusinessEmail(), CreateBusinessPhoneNumber());
    ...
    private static UpdateCompanyContactInfoCommand CreateCommand(Guid companyId, string newBusinessEmail,
        string newBusinessPhoneNumber)
    {
        return new UpdateCompanyContactInfoCommand
        {
            ...
            NewBusinessPhoneNumber = newBusinessPhoneNumber
        };
    }
