...
public class UpdateGuestContactInfoUseCaseTests
{
    private readonly IGuestsRepository _guestsRepositoryMock;
    private readonly IUnitOfWork _unitOfWorkMock;
    private readonly UpdateGuestContactInfoUseCase _useCase;

    public UpdateGuestContactInfoUseCaseTests()
    {
        _guestsRepositoryMock = Substitute.For<IGuestsRepository>();
        _unitOfWorkMock = Substitute.For<IUnitOfWork>();
        _useCase = new UpdateGuestContactInfoUseCase(_guestsRepositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task Handle_UpdatesGuestContactInfo()
    {
        // Arrange
        var guestId = Guid.NewGuid();
        var guest = CreateGuest(guestId);
        var command = CreateCommand(guestId, "guest.updated@example.com", "+18095550000");
        ...
        // Assert
        guest.Email.Value.Should().Be(command.Email);
        guest.PhoneNumber.Value.Should().Be(command.PhoneNumber);
