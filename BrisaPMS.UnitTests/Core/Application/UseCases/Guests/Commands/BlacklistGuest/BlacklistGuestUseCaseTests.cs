...
public class BlacklistGuestUseCaseTests
{
    private readonly IGuestsRepository _guestsRepositoryMock;
    private readonly IUnitOfWork _unitOfWorkMock;
    private readonly BlacklistGuestUseCase _useCase;

    public BlacklistGuestUseCaseTests()
    {
        _guestsRepositoryMock = Substitute.For<IGuestsRepository>();
        _unitOfWorkMock = Substitute.For<IUnitOfWork>();
        _useCase = new BlacklistGuestUseCase(_guestsRepositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task Handle_BlacklistsGuest()
    {
        // Arrange
        var guestId = Guid.NewGuid();
        var guest = CreateGuest(guestId);
        var command = CreateCommand(guestId, "Repeated property damage");
        ...
        guest.BlackListedReason.Should().Be(command.BlacklistedReason);
