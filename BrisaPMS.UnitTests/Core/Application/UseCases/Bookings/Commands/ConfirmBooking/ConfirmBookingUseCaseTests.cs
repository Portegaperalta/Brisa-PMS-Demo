...
public class ConfirmBookingUseCaseTests
{
    ...
    public ConfirmBookingUseCaseTests()
    {
        ...
        _useCase = new ConfirmBookingUseCase(_bookingsRepositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task Handle_ConfirmsBookingAndReturnsTrue()
    {
        // Arrange
    ...
    [Fact]
    public async Task Handle_ThrowsBusinessRuleException_WhenBookingIsAlreadyConfirmed()
    {
        // Arrange
        var hotelId = Guid.NewGuid();
        var roomId = Guid.NewGuid();
    ...
    public async Task Handle_ThrowsBusinessRuleException_WhenBookingIsAlreadyCompleted()
    {
        ...
        // Assert
        await act.Should().ThrowAsync<BusinessRuleException>()
            .WithMessage("Completed booking can't be set as confirmed");
    }
    ...
    public async Task Handle_ThrowsBusinessRuleException_WhenBookingIsCancelled()
    {
        ...
        // Assert
        await act.Should().ThrowAsync<BusinessRuleException>()
            .WithMessage("Cancelled booking can't be set as confirmed");
    }
    ...
    private static ConfirmBookingCommand CreateValidCommand(Guid bookingId)
    {
    ...
    private static Booking CreateBooking(Guid hotelId, Guid roomId, BookingStatus status = BookingStatus.Pending)
    {
