...
public class UpdateCheckInOutTimesUseCaseTests
{
    private readonly IBookingsRepository _bookingsRepositoryMock;
    private readonly IUnitOfWork _unitOfWorkMock;
    private readonly UpdateCheckInOutTimesUseCase _useCase;

    public UpdateCheckInOutTimesUseCaseTests()
    {
        _bookingsRepositoryMock = Substitute.For<IBookingsRepository>();
        _unitOfWorkMock = Substitute.For<IUnitOfWork>();
        _useCase = new UpdateCheckInOutTimesUseCase(_bookingsRepositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task Handle_UpdatesCheckInOutTimesAndReturnsTrue()
    {
        // Arrange
        var hotelId = Guid.NewGuid();
        var roomId = Guid.NewGuid();
        var bookingId = Guid.NewGuid();
        var newCheckIn = new DateTime(2026, 4, 25, 15, 0, 0, DateTimeKind.Utc);
        var newCheckOut = new DateTime(2026, 4, 27, 11, 0, 0, DateTimeKind.Utc);
        var command = CreateValidCommand(bookingId, newCheckIn, newCheckOut);
        ...
        // Assert
        booking.CheckInOutTimes.CheckInTime.Should().Be(newCheckIn);
        booking.CheckInOutTimes.CheckOutTime.Should().Be(newCheckOut);
    ...
    public async Task Handle_CallsBookingsRepository()
    {
        ...
        var command = CreateValidCommand(bookingId,
            new DateTime(2026, 4, 25, 15, 0, 0, DateTimeKind.Utc),
            new DateTime(2026, 4, 27, 11, 0, 0, DateTimeKind.Utc));
        var booking = CreateBooking(hotelId, roomId);
    ...
    public async Task Handle_ThrowsBusinessRuleException_WhenBookingIsCompleted()
    {
        ...
        await act.Should().ThrowAsync<BusinessRuleException>()
            .WithMessage("Booking is already completed, unable to modify CheckIn-Out Times");
    }
    ...
    public async Task Handle_ThrowsBusinessRuleException_WhenBookingIsCancelled()
    {
        ...
        // Assert
        await act.Should().ThrowAsync<BusinessRuleException>()
            .WithMessage("Booking is already cancelled, unable to modify CheckIn-Out Times");
    }
    ...
    private static UpdateCheckInOutTimesCommand CreateValidCommand(Guid bookingId, DateTime checkInTime, DateTime checkOutTime)
    {
