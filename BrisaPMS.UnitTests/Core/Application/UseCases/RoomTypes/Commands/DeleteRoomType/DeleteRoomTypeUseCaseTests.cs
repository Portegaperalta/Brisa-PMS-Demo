using BrisaPMS.Application.Contracts.Persistence;
using BrisaPMS.Application.Contracts.Repositories;
using BrisaPMS.Application.Exceptions;
using BrisaPMS.Application.UseCases.RoomTypes.Commands.DeleteRoomType;
using BrisaPMS.Domain.Billing;
using BrisaPMS.Domain.RoomTypes;
using FluentAssertions;
using NSubstitute;
using NSubstitute.ExceptionExtensions;

namespace BrisaPMS.UnitTests.Core.Application.UseCases.RoomTypes.Commands.DeleteRoomType;

public class DeleteRoomTypeUseCaseTests
{
    private readonly IRoomTypesRepository _repositoryMock;
    private readonly IUnitOfWork _unitOfWorkMock;
    private readonly DeleteRoomTypeUseCase _useCase;

    public DeleteRoomTypeUseCaseTests()
    {
        _repositoryMock = Substitute.For<IRoomTypesRepository>();
        _unitOfWorkMock = Substitute.For<IUnitOfWork>();
        _useCase = new DeleteRoomTypeUseCase(_repositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task Handle_DeletesRoomType()
    {
        // Arrange
        var roomTypeId = Guid.NewGuid();
        var roomType = CreateRoomType(roomTypeId);
        var command = new DeleteRoomTypeCommand { Id = roomTypeId };

        _repositoryMock.GetById(roomTypeId).Returns(roomType);

        // Act
        var result = await _useCase.Handle(command);

        // Assert
        await _repositoryMock.Received(1).Delete(roomType);
        await _unitOfWorkMock.Received(1).Persist();
        await _unitOfWorkMock.DidNotReceive().Revert();
        result.Should().BeTrue();
    }

    [Fact]
    public async Task Handle_ThrowsNotFoundException_WhenRoomTypeDoesNotExist()
    {
        // Arrange
        var command = new DeleteRoomTypeCommand { Id = Guid.NewGuid() };

        _repositoryMock.GetById(command.Id).Returns((RoomType?)null);

        // Act
        var act = async () => await _useCase.Handle(command);

        // Assert
        await act.Should().ThrowAsync<NotFoundException>();
        await _repositoryMock.DidNotReceive().Delete(Arg.Any<RoomType>());
        await _unitOfWorkMock.DidNotReceive().Persist();
        await _unitOfWorkMock.DidNotReceive().Revert();
    }

    [Fact]
    public async Task Handle_RevertsUnitOfWork_WhenRepositoryDeleteFails()
    {
        // Arrange
        var roomTypeId = Guid.NewGuid();
        var roomType = CreateRoomType(roomTypeId);
        var command = new DeleteRoomTypeCommand { Id = roomTypeId };

        _repositoryMock.GetById(roomTypeId).Returns(roomType);
        _repositoryMock.Delete(Arg.Any<RoomType>()).Throws<InvalidOperationException>();

        // Act
        var act = async () => await _useCase.Handle(command);

        // Assert
        await act.Should().ThrowAsync<InvalidOperationException>();
        await _unitOfWorkMock.Received(1).Revert();
        await _unitOfWorkMock.DidNotReceive().Persist();
    }

    private static RoomType CreateRoomType(Guid? roomTypeId = null)
    {
        return new RoomType(
            "Deluxe Suite",
            new RoomBaseRate(25m),
            new RoomBed(BedType.Double, 2),
            new OccupancyPolicy(2, 1),
            "Spacious suite with ocean view")
        {
            Id = roomTypeId ?? Guid.NewGuid()
        };
    }
}
