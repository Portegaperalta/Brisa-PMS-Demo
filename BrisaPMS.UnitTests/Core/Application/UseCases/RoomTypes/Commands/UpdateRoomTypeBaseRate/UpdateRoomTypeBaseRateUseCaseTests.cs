...
public class UpdateRoomTypeBaseRateUseCaseTests
{
    private readonly IRoomTypesRepository _repositoryMock;
    private readonly IUnitOfWork _unitOfWorkMock;
    private readonly UpdateRoomTypeBaseRateUseCase _useCase;

    public UpdateRoomTypeBaseRateUseCaseTests()
    {
        _repositoryMock = Substitute.For<IRoomTypesRepository>();
        _unitOfWorkMock = Substitute.For<IUnitOfWork>();
        _useCase = new UpdateRoomTypeBaseRateUseCase(_repositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task Handle_UpdatesRoomTypeBaseRate()
    {
        // Arrange
        var roomTypeId = Guid.NewGuid();
        var command = CreateCommand(roomTypeId, 30m);
        ...
        // Assert
        roomType.BaseRate.Rate.Should().Be(command.NewBaseRate);
    ...
    {
        // Arrange
        var command = CreateCommand(Guid.NewGuid(), 30m);
